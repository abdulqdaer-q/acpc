      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="grid md:grid-cols-2 gap-8">
          {stages.map((stage, index) => {
            const feeKey = `stages.${stage}.fee`;
            const locationKey = `stages.${stage}.location`;
            const costsKey = `stages.${stage}.costs`;
            const qualificationKey = `stages.${stage}.qualification`;
            ...
            return (
              <div key={stage} className="relative bg-gradient-to-br from-primary-50 to-white p-8 rounded-lg shadow-lg border-2 border-primary-100 hover:shadow-xl transition-shadow">
                ...
                <div className="space-y-2 text-sm">
                  ...
                  {hasKey(qualificationKey) && (
                    <div className="flex items-start">
                      <svg className="w-5 h-5 text-primary-600 ml-2 mt-0.5" fill="currentColor" viewBox="0 0 20 20">
                        <path fillRule="evenodd" d="M10 18a8 8 0 100-16 8 8 0 000 16zm3.707-9.293a1 1 0 00-1.414-1.414L9 10.586 7.707 9.293a1 1 0 00-1.414 1.414l2 2a1 1 0 001.414 0l4-4z" clipRule="evenodd" />
                      </svg>
                      <span className="text-gray-600">{t(qualificationKey)}</span>
                    </div>
                  )}
                  ...
                  {hasKey(feeKey) && (
                    <div className="flex items-start">
                      ...
                      <span className="text-amber-700 font-semibold">{t(feeKey)}</span>
                    </div>
                  )}
                  ...
                  {hasKey(locationKey) && (
                    <div className="flex items-start">
                      ...
                      <span className="text-gray-600">{t(locationKey)}</span>
                    </div>
                  )}
                  ...
                  {hasKey(costsKey) && (
                    <div className="flex items-start">
                      ...
                      <span className="text-green-700">{t(costsKey)}</span>
                    </div>
                  )}
                </div>
              </div>
